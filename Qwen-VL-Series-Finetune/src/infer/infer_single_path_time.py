...
def main(args):
    ...
    # Throughput: wall time of model.generate (includes retries); tokens = new tokens only on saved outputs
    total_new_tokens = 0
        ...
        with open(answer_file, "w") as ans_file:
            for i, (conversation,question, answer,video_name, daily_forecast) in enumerate(tqdm(dataloader)):
                ...
                while not all_passed:
                    # Inference (timed; sync GPU for meaningful wall time)
                    ...
                    if torch.cuda.is_available():
                        torch.cuda.synchronize()
                    dt_gen = time.perf_counter() - t_gen0
                    total_gen_seconds += dt_gen

                    generated_ids_trimmed = [
                        out_ids[len(in_ids) :] for in_ids, out_ids in zip(inputs.input_ids, generated_ids)
                    ]
                    batch_new_tokens = sum(len(t) for t in generated_ids_trimmed)
                    ...
                    if all_passed:
                        ...
                        tps = batch_new_tokens / dt_gen if dt_gen > 0 else 0.0
                        print(
                            f"[infer speed] batch {i} area={area} new_tokens={batch_new_tokens} "
                            f"gen_time={dt_gen:.3f}s tokens/s={tps:.1f}"
                        )
                        batch_results = []
                        for j in range(batch_size):
                            qa = {
    ...
    if total_gen_seconds > 0:
        avg_tps_all_generate = total_new_tokens / total_gen_seconds
        print(
            f"[infer speed] summary: saved_new_tokens={total_new_tokens} "
            f"total_generate_wall_s={total_gen_seconds:.2f} "
            f"avg_tokens/s (new tokens / all generate calls)={avg_tps_all_generate:.1f} "
            f"saved_batches={total_saved_batches}"
        )
